...
In the actual implementation, both views are stacked horizontally to form a single rectangular image of size 1758x400 (WxH). Then, in the custom PyTorch dataset class, the stacked images are resized to 400x100 pixels to gain performance in terms of speed and memory usage.
...
![D2 action on a small grid](docs/d2_action_on_grid.png)

**Figure:** Cayley graph for $D_2$ showing how the group acts on a rectangle. This visualization is useful to keep in mind later when we consider group equivariant layers.
...
Recall that in the implentation, *both views are combined* by stacking them horizontally, The $D_2$ group actions are simply implemented using `torch.flip()` and also supports mini-batches.
